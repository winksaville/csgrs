...
fn main() {
    ...
    let _ = fs::write(
        "stl/cube_grow_0_2.stl",
        grown_cube.to_stl_binary("cube_grow_0_2").unwrap(),
    );

    let shrunk_cube = cube.shrink(0.2); // approximate inward offset
    let _ = fs::write(
        "stl/cube_shrink_0_2.stl",
        shrunk_cube.to_stl_binary("cube_shrink_0_2").unwrap(),
    );
    ...
    let _ = fs::write(
        ...
        grown_2d.to_stl_binary("square_2d_grow_0_5").unwrap(),
    );
    ...
    let _ = fs::write(
        "stl/tetrahedron.stl",
        poly.to_stl_binary("tetrahedron").unwrap(),
    );
    ...
    let text_csg = MyCSG::text("HELLO", font_data, Some(15.0));
    let _ = fs::write(
    ...
    let _ = fs::write(
        "stl/retriangulated.stl",
        retriangulated_shape
            .to_stl_binary("retriangulated")
            .unwrap(),
    );
